...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        if(__wine_init_unix_call()){
            ...
            // fprintf(stdout,"wine unix call failed\n");
            puts("failed unix call init\n");
            return FALSE;
        }
        dbg_report("__wine_init_unix_call success\n");
 #endif
...
#ifdef NP_WINE_UNIX_LIB
NP_EXPORT(int) OpentrackUnixLibVersion()
{
    return 1;
}
#endif
...
static bool FTCreateMapping(void)
{
    if (pMemData)
        return true;
#ifdef NP_WINE_UNIX_LIB
    if(WINE_UNIX_CALL(otr_init,NULL)) {
        dbg_report("otr_init failed\n");
        return false;
    }
    pMemData = &MemData;
#else
