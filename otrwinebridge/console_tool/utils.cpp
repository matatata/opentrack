...
void printUsage(const wstring& progname) {
    wcout << L"Usage: " << progname << L" cmd1 cmd2 ... cmdN  where cmd is one of -h, -i, -v, -p, -x, -e ft|np|all, -d ft|np|all, -c or-l <path>" << endl;
}


void printHelp(const wstring& progname) {
    wcout << L"=== OTR Wine Bridge Console Tool ===" << endl;
    wcout << L"This program manages the registry entries that dictate where games will look for the freetrackclient and NPCLient dlls." << endl;
    printUsage(progname);
    ...
    wcout << L"  -l C:\\custom\\location\\          Set a custom location. Pass before any occurence of -e to be effective." << endl;
    ...
    wcout << endl;
    wcout << L"By default the location for the client dlls is assumed to this program's location. Use -l and -c to modify this behavior" << endl;
    wcout << endl;
...
{
    wcout << L"=== Interactive Mode ===" << endl;
    wcout << L"[n] Enable NPClient protocol exclusively" << endl;
    wcout << L"[f] Enable FreeTrackClient protocol exclusively" << endl;
    wcout << L"[b] Enable both protocols" << endl;
