...
/**
 * return 0 if tracking stopped
 */
int OTR_GetFTHeap(FTHeap* ftheap)
{
    // not aquiring a lock, as the original opentrack-wrapper-wine
    // did not do it either. Need to think about it.
    // OTR_Priv_Lock();
    ...
    return shm_posix->stop ? 0 : 1;
}


static NTSTATUS unix_OTR_GetFTHeap(void *args)
{
   return (NTSTATUS)OTR_GetFTHeap((FTHeap*)args);
}
